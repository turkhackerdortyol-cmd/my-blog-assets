...
/* ================================================================
   === PWA KURULUM BANNER'I (Ana Ekrana Ekle) ===
   Akış: beforeinstallprompt yakalanır -> özel banner (teyit.org tarzı)
   gösterilir -> "Şimdi Yükle" tıklanınca native tarayıcı kurulum
   penceresi (deferredPrompt.prompt()) tetiklenir.
   Not: Bu mantığın çalışması için /manifest.json ve /sw.js dosyalarının
   blogun KENDİ alan adından (Cloudflare Worker ile) servis edilmesi şarttır.
   ================================================================ */
(function(){
  var VB_PWA_LATER_KEY = 'vbPwaLaterUntil';
  var VB_PWA_DONE_KEY  = 'vbPwaDone';
  var VB_PWA_ICON = 'https://turkhackerdortyol-cmd.github.io/my-blog-assets/1000589759-photoaidcom-cropped.png';
  var deferredPrompt = null;

  function vbPwaIsStandalone(){
    try{
      return (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches) || window.navigator.standalone === true;
    }catch(e){ return false; }
  }
  function vbPwaShouldOffer(){
    if (vbPwaIsStandalone()) return false;
    try{
      if (localStorage.getItem(VB_PWA_DONE_KEY)) return false;
      var later = parseInt(localStorage.getItem(VB_PWA_LATER_KEY) || '0', 10);
      if (later && Date.now() < later) return false;
    }catch(e){}
    return true;
  }
  function vbPwaSiteName(){
    try{
      /* Öncelik: Blogger'ın data:blog.title değerinden head'e yazılan
         window.VB_SITE_NAME (her sayfada, konu başlığından bağımsız olarak
         her zaman doğru site adını verir). Bulunamazsa document.title'dan
         kaba bir tahmin yapılır (yedek yol). */
      if (window.VB_SITE_NAME) return String(window.VB_SITE_NAME).trim();
      var t = document.title || '';
      return (t.split(/[-|·]/)[0] || 'Uygulama').trim();
    }catch(e){ return 'Uygulama'; }
  }
  function vbPwaBuildBanner(){
    var existing = document.getElementById('vbPwaBanner');
    if (existing) return existing;
    var el = document.createElement('div');
    el.id = 'vbPwaBanner';
    el.className = 'vb-pwa-banner vb-pwa-hide';
    el.setAttribute('role','dialog');
    el.setAttribute('aria-label','Uygulamayı yükle');
    el.innerHTML =
      '<button type="button" class="vb-pwa-close" id="vbPwaCloseBtn" aria-label="Kapat">&#215;</button>' +
      '<div class="vb-pwa-icon"><img src="'+ VB_PWA_ICON +'" alt="" loading="lazy"/></div>' +
      '<div class="vb-pwa-text">' +
        '<div class="vb-pwa-title">'+ safeText(vbPwaSiteName()) +' artık cebinde!</div>' +
        '<div class="vb-pwa-desc" id="vbPwaDesc">Uygulamayı ana ekranına ekle, her yerden hızlıca eriş.</div>' +
      '</div>' +
      '<div class="vb-pwa-actions">' +
        '<button type="button" class="vb-pwa-btn-install" id="vbPwaInstallBtn">Şimdi Yükle</button>' +
        '<button type="button" class="vb-pwa-btn-later" id="vbPwaLaterBtn">Başka zaman</button>' +
      '</div>';
    document.body.appendChild(el);

    function dismissLater(){
      try{ localStorage.setItem(VB_PWA_LATER_KEY, String(Date.now() + 14*24*60*60*1000)); }catch(e){}
      vbPwaHide();
    }
    document.getElementById('vbPwaLaterBtn').addEventListener('click', dismissLater);
    document.getElementById('vbPwaCloseBtn').addEventListener('click', dismissLater);
    document.getElementById('vbPwaInstallBtn').addEventListener('click', function(){
      if (!deferredPrompt){ vbPwaHide(); return; }
      deferredPrompt.prompt();
      deferredPrompt.userChoice.then(function(choice){
        if (choice && choice.outcome === 'accepted'){
          try{ localStorage.setItem(VB_PWA_DONE_KEY, '1'); }catch(e){}
        }
        deferredPrompt = null;
        vbPwaHide();
      });
  ...
  function vbPwaShow(){
    if (!vbPwaShouldOffer()) return;
    var el = vbPwaBuildBanner();
    el.classList.remove('vb-pwa-hide');
    document.body.classList.add('vb-pwa-open');
  }
  function vbPwaHide(){
    var el = document.getElementById('vbPwaBanner');
    if (el) el.classList.add('vb-pwa-hide');
    document.body.classList.remove('vb-pwa-open');
  }
  function vbPwaIsIos(){
    var ua = navigator.userAgent;
    if (/iphone|ipad|ipod/i.test(ua)) return true;
    /* iPadOS 13+ Safari, masaüstü Safari gibi "Macintosh" User-Agent'ı
       gönderir; dokunmatik nokta sayısına bakarak bunu da yakala. */
    try{
      return navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1;
    }catch(e){ return false; }
  }
  function vbPwaIsSafari(){
    return /^((?!chrome|android|crios|fxios).)*safari/i.test(navigator.userAgent);
  }

  /* Chrome/Edge/Android: native kurulum istemini yakala */
  window.addEventListener('beforeinstallprompt', function(e){
    e.preventDefault();
    deferredPrompt = e;
    setTimeout(vbPwaShow, 1200);
  });
  ...
  /* iOS Safari: beforeinstallprompt desteklenmez, manuel talimat göster */
  document.addEventListener('DOMContentLoaded', function(){
    if (vbPwaIsIos() && vbPwaIsSafari() && vbPwaShouldOffer()){
      setTimeout(function(){
        var el = vbPwaBuildBanner();
        var desc = document.getElementById('vbPwaDesc');
        if (desc) desc.textContent = 'Yüklemek için altta ki paylaş simgesine dokun ve "Ana Ekrana Ekle" seçeneğini seç.';
        var installBtn = document.getElementById('vbPwaInstallBtn');
  ...
  /* Service Worker kaydı (aynı origin üzerinden /sw.js) */
